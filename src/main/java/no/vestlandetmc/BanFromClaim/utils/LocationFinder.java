...
public class LocationFinder {
    ...
    public void IterateCircumferences(CallbackReturnLocation callback) {
        ...
        Bukkit.getScheduler().runTask(plugin, () -> {
            ...
            outer:
            for (int i = 0; i < maxCircleIterations; i++) {
                circumferenceRadius *= 2;

                for (int j = 0; j < checkLocationsPerCircumference; j++) {
                    randomCircumferenceRadiusLoc = GetRandomCircumferenceLoc(
                            this.circumferenceCenter, circumferenceRadius, circumferenceWorld);
                    ...
                    if (!hasClaim(randomCircumferenceRadiusLoc)) {
                        ...
                        if (SafeLocationCheck.BlockSafetyCheck(highestBlock)) {
                            randomCircumferenceRadiusLoc = new Location(
                                    circumferenceWorld,
                                    highestBlock.getX() + 0.5,
                                    highestBlock.getY() + 1,
                                    highestBlock.getZ() + 0.5
                            );
                            break outer;
            ...
            callback.onDone(randomCircumferenceRadiusLoc);
        });
    }
